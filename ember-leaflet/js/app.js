...
App.Router.map(function() {
  ...
  this.resource('leaflet',function(){
  })
  this.route('map1')
  this.route('map2')
});
...
App.Leaflet1View = EmberLeaflet.MapView.extend({});

var StamenURLS = App.Store.create({
  toner: 'http://d.tile.stamen.com/toner/{z}/{x}/{y}.png',
  watercolor: 'http://d.tile.stamen.com/watercolor/{z}/{x}/{y}.png',
  terrain: 'http://d.tile.stamen.com/terrain/{z}/{x}/{y}.png'
})

App.TonerTileLayer = EmberLeaflet.DefaultTileLayer.extend({
  tileUrl: StamenURLS.get("toner")
});

App.WatercolorTileLayer = EmberLeaflet.DefaultTileLayer.extend({
  tileUrl: StamenURLS.get("watercolor")
});

App.TerrainTileLayer = EmberLeaflet.DefaultTileLayer.extend({
  tileUrl: StamenURLS.get("terrain"),
  options:{"minZoom":5}
});
...
function build_clusters(){
    ...
    for (var i = 0; i < addressPoints.length; i++) {
      ...
      var title = a[2];
      var marker = {"location":[a[0], a[1]]};
      markers.push(marker);
    }
    return markers
}

var globalClusterLayer = new L.MarkerClusterGroup();

App.MarkerCollectionLayer = EmberLeaflet.MarkerCollectionLayer.extend({
  content: function(){ return build_clusters() }.property()
});
...
App.MarkerClusterLayer =
  EmberLeaflet.ContainerLayer.extend({
    childLayers: [
      ...
      _newLayer: function() {
        return globalClusterLayer; 
      }
});


App.NYMarkerCollectionLayer = EmberLeaflet.MarkerCollectionLayer.extend({
    content: [
...
App.NYMarkerClusterLayer = EmberLeaflet.ContainerLayer.extend({
    childLayers: [App.NYMarkerCollectionLayer],
      _newLayer: function() {
        return globalClusterLayer; 
      }
});

App.LeafletRoute = Ember.Route.extend({
  model:function(){
    console.log('okok')
    return "Leaflet Example"
  }
})

App.LeafletController = Ember.Controller.extend({
  zoomLevel: 5,
  map: null,
...
App.LeafletView = EmberLeaflet.MapView.extend({
  childLayers: [
  ...
  base_url: 'https://{s}.tiles.mapbox.com/v3/examples.map-zr0njcqy/{z}/{x}/{y}.png',
  
  base_layers: ["toner","watercolor","terrain"],
  
  clusterLayer: globalClusterLayer,
  ...
  didCreateLayer: function() {
    ...
    var drawnItems = this.get("clusterLayer");
    ...
    var base = {"Default":L.tileLayer(this.get("base_url"),options)},
        base_layers = this.get("base_layers");
    
    var terrian_alt = "Terrian (US ONLY)";
    
    $(base_layers).each(function(){
      var title = this.charAt(0).toUpperCase() + this.substring(1);
      var opt = options;
      if(this == "terrain"){
        title = terrian_alt;
        opt["minZoom"] = 5;
      }
      base[title] = L.tileLayer(StamenURLS.get(this),opt);
    })
      
    L.control.layers(base).addTo(map);
    
    $("form.leaflet-control-layers-list input").first().prop("checked", true);
    
    
    map.on('baselayerchange',function(e){
      // console.log(e.name);
      if(e.name == terrian_alt){
        if(map.getZoom>4){
          map.setZoom(4);
          map.panTo( L.latLng(37.8, -96));
        }
      }
    })
    
    /* Testing Leaflet Drawing events */
    map.on('draw:created', function (e) {
      var type = e.layerType,
        layer = e.layer;

      if (type === 'marker') {
        latlng = layer._latlng.toString();
        layer.bindPopup(latlng);
      }
    ...
    /* end test of leaflet drawing */
  }
});


App.Map1Route = Ember.Route.extend({
  model:function(){
    return "Leaflet Example #1"
  }
})

App.Map1View = App.LeafletView.extend({
  childLayers:[
      App.WatercolorTileLayer,
      App.NYMarkerClusterLayer,
      App.MarkerClusterLayer
  ],
  base_url: StamenURLS.get('watercolor'),
  base_layers:["toner","terrain"]
});


App.Map2Route = Ember.Route.extend({
  model:function(){
    return "Leaflet Example #2"
  }
})

App.Map2View = App.LeafletView.extend({
  childLayers:[
      App.TonerTileLayer,
      App.NYMarkerClusterLayer,
      App.MarkerClusterLayer
  ...
  base_url: '',
  base_layers:["watercolor"],
  didCreateLayer:function(){
    this._super();
    var stateData = $.getJSON("js/data/us-states.json")
    var map = this._layer;
    stateData.success(function(data){
